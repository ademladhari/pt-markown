...
class DDIMInverter:
    ...
    def image_to_latent(self, image_tensor: torch.Tensor) -> torch.Tensor:
        ...
        return posterior.sample() * self.core.pipe.vae.config.scaling_factor
    ...
    def invert(self, prompt: str, image_tensor: torch.Tensor) -> List[torch.Tensor]:
        ...
        for t in reversed(scheduler.timesteps):
            ...
            latent_input = torch.cat([cur, cur], dim=0)
            ...
            noise_guided = noise_uncond + self.core.cfg.inversion_guidance_scale * (noise_cond - noise_uncond)
            ...
            next_alpha = alpha_t if t_idx == first_idx else scheduler.alphas_cumprod[next_idx]
            cur = (cur - (1 - alpha_t).sqrt() * noise_guided) / alpha_t.sqrt()
            cur = next_alpha.sqrt() * cur + (1 - next_alpha).sqrt() * noise_guided
